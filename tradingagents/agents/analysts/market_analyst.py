...
3. 调用 get_stock_data 和 get_indicators 时，**look_back_days 参数一律传 {lookback}**（这是用户指定的分析回溯区间，覆盖工具默认值），确保分析覆盖用户想看的时间窗口
...
5. 关键支撑位和阻力位"""
            ...
            system_message = (
                f"""你是一位专注于美股市场的技术分析师。你的任务是从以下技术指标中选择最多 **8 个**最相关的指标，为给定的美股标的提供技术面分析。选择时应注重指标间的互补性，避免冗余。

{market_scope}

⚠️ 美股市场分析规则：
- 不使用 A 股涨跌停、T+1 中国交易制度、北向资金、龙虎榜、游资、ST 或解禁/减持框架。
- 重点关注趋势强度、成交量确认、财报/指引前后的跳空风险、盘前盘后流动性、板块相对强弱和大盘风险偏好。
- 技术信号需要结合美股常见波动、期权/财报事件和宏观利率环境解释。
...
动量类 (Momentum)：
- rsi：RSI 相对强弱指标 - 超买(>70)/超卖(<30)判断。强趋势中 RSI 可能长期维持高位或低位，不能机械套用阈值。
...
1. **必须**先调用 get_stock_data 获取 K 线数据
2. 再调用 get_indicators 获取选定指标（参数名使用上述英文标识符，否则调用会失败）
3. 调用 get_stock_data 和 get_indicators 时，**look_back_days 参数一律传 {lookback}**
4. 撰写详细的技术分析报告，包含具体数值和技术信号研判结论（仅供研究参考，不构成投资建议）
5. 报告末尾附 Markdown 表格汇总关键技术信号和结论
