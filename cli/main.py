...
from tradingagents.agents.utils.agent_utils import is_chinese_output, report_label
...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    if final_state.get("market_report"):
        ...
        analyst_parts.append((report_label("Market Analyst"), final_state["market_report"]))
    if final_state.get("sentiment_report"):
        analysts_dir.mkdir(exist_ok=True)
        (analysts_dir / "sentiment.md").write_text(final_state["sentiment_report"], encoding="utf-8")
        analyst_parts.append((report_label("Social Analyst"), final_state["sentiment_report"]))
    if final_state.get("news_report"):
        analysts_dir.mkdir(exist_ok=True)
        (analysts_dir / "news.md").write_text(final_state["news_report"], encoding="utf-8")
        analyst_parts.append((report_label("News Analyst"), final_state["news_report"]))
    if final_state.get("fundamentals_report"):
        analysts_dir.mkdir(exist_ok=True)
        (analysts_dir / "fundamentals.md").write_text(final_state["fundamentals_report"], encoding="utf-8")
        analyst_parts.append((report_label("Fundamentals Analyst"), final_state["fundamentals_report"]))
    if analyst_parts:
        content = "\n\n".join(f"### {name}\n{text}" for name, text in analyst_parts)
        sections.append(f"## I. {report_label('Analyst Team Reports')}\n\n{content}")
    ...
    if final_state.get("investment_debate_state"):
        ...
        if debate.get("bull_history"):
            ...
            research_parts.append((report_label("Bull Researcher"), debate["bull_history"]))
        if debate.get("bear_history"):
            research_dir.mkdir(exist_ok=True)
            (research_dir / "bear.md").write_text(debate["bear_history"], encoding="utf-8")
            research_parts.append((report_label("Bear Researcher"), debate["bear_history"]))
        if debate.get("judge_decision"):
            research_dir.mkdir(exist_ok=True)
            (research_dir / "manager.md").write_text(debate["judge_decision"], encoding="utf-8")
            research_parts.append((report_label("Research Manager"), debate["judge_decision"]))
        if research_parts:
            content = "\n\n".join(f"### {name}\n{text}" for name, text in research_parts)
            sections.append(f"## II. {report_label('Research Team Decision')}\n\n{content}")
    ...
    if final_state.get("trader_investment_plan"):
        ...
        sections.append(f"## III. {report_label('Trading Team Plan')}\n\n### {report_label('Trader')}\n{final_state['trader_investment_plan']}")
    ...
    if final_state.get("risk_debate_state"):
        ...
        if risk.get("aggressive_history"):
            ...
            risk_parts.append((report_label("Aggressive Analyst"), risk["aggressive_history"]))
        if risk.get("conservative_history"):
            risk_dir.mkdir(exist_ok=True)
            (risk_dir / "conservative.md").write_text(risk["conservative_history"], encoding="utf-8")
            risk_parts.append((report_label("Conservative Analyst"), risk["conservative_history"]))
        if risk.get("neutral_history"):
            risk_dir.mkdir(exist_ok=True)
            (risk_dir / "neutral.md").write_text(risk["neutral_history"], encoding="utf-8")
            risk_parts.append((report_label("Neutral Analyst"), risk["neutral_history"]))
        if risk_parts:
            content = "\n\n".join(f"### {name}\n{text}" for name, text in risk_parts)
            sections.append(f"## IV. {report_label('Risk Management Team Decision')}\n\n{content}")
        ...
        if risk.get("judge_decision"):
            ...
            sections.append(f"## V. {report_label('Portfolio Manager Decision')}\n\n### {report_label('Portfolio Manager')}\n{risk['judge_decision']}")

    # Write consolidated report
    if is_chinese_output():
        header = f"# {report_label('Trading Analysis Report')}：{ticker}\n\n{report_label('Generated')}：{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n> ⚠️ 免责声明：本报告由 AI 自动生成，仅供学习研究与技术演示，不构成任何投资建议。投资有风险，决策请咨询持牌专业机构。\n\n"
    else:
